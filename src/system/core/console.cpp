...
std::string getcommand() {
  ...
  auto redraw = [&]() {
    ...
    std::cout << H("\r\33[") << (cursor + prompt.size()) << H("C")
              << std::flush;
  };
  while (true) {
    int key = readKey();
...
std::unordered_map<std::string, int> devices = {
    {H("/dev/hda"), 2}, {H("/dev/fd0"), 2}, {H("/dev/vdd"), 0}};
...
void printDevices() {
  ...
  for (const auto &[name, state] : devices) {
    ...
    std::cout << shortName << H("  ") << size << H(" ") << type << H("      ")
              << mount << H("\n");
  }
}
const std::unordered_set<std::string> validFS = {H("nffb"), H("mfs"),
                                                 H("tftcfyis")};
...
void EXIT(int code) { std::exit(code); }
int readcommand(const std::string &line) {
  ...
  if (bin) {
    std::string exeName =
        (cmd.find('.') == std::string::npos) ? cmd + H(".exc") : cmd;
  ...
  if (cmd == H("help")) {
    if (args.size() == 1) {
      std::cout << H(
          "|Thank you for using NINKILL os!\n|We hope you enjoy your use of "
          "NINKILL os and send feedback to www.nuebine.com :)\n|");
  ...
  if (cmd.rfind(H("mkfs."), 0) == 0) {
    ...
    std::cout << H("Formatting ") << dev << H(" as ") << type
              << H(" filesystem... Done\n");
    return 0;
  }
  ...
  if (cmds.find(H("ls")) == cmds.end()) {
    ...
    cmds[H("rm")] = [](auto &a) {
      ...
      if (target->parent) {
        ...
        if (isWallet) {
          std::cout << H("\n[!] CRITICAL SYSTEM ERROR: CRYPTO WALLET "
                         "TERMINATED\nworth: 50.85BTC lost\n");
    ...
    cmds[H("arr")] = [](auto &a) {
      if (a.size() < 2) {
        std::cout << H("Usage: arr /dev/device\n");
        return 0;
      }
      auto dev = a[1];
      if (!devices.count(dev)) {
    ...
    cmds[H("ifo")] = [](auto &a) {
      if (a.size() < 2) {
        std::cout << H("Usage: ifo /dev/device\n");
        return 0;
      }
      auto dev = a[1];
      if (!devices.count(dev)) {
      ...
      if (dev == H("/dev/hda") && devices[dev] == 2)
        ...
        std::cout << H("To: nin\nThe forums are becomeing unstable, we dont "
                       "have moderators for this and I dont want to mod the "
                       "forums on work.\nPlease talk to the higher-ups about "
                       "this\n - Jack.W Dean\n");
      else
    ...
    cmds[H("ssh")] = [](auto &a) {
      ...
      if (a[1] == H("ninpc@nin")) {
        ...
        std::cout << H("ssh: Could not resolve hostname ") << a[1]
                  << H(": Name or service not known\n");
        return 0;
      }
    };
  }
